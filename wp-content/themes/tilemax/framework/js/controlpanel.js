...
jQuery(document).ready(function($){
	...
	if($.cookie("tilemax_skin")!= null){
		var $href = dttheme_urls.theme_base_url + '/css/skins/' + $.cookie("tilemax_skin")+"/style.css";
		$("link[id='skin-css']").attr("href",$href);
		$("ul.color-picker a[id='"+$.cookie("tilemax_skin")+"']").addClass("selected");
	}else{
		$("ul.color-picker a:first").addClass("selected");
	}

	//Apply Layout
	if($.cookie("tilemax_layout") == "boxed"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("tilemax_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");

		$i = ($.cookie("tilemax_pattern")) ? $.cookie("tilemax_pattern")  : 'pattern1';
		...
		$("ul.pattern-picker a[id="+$.cookie("tilemax_pattern")+"]").addClass('selected');
	} else if($.cookie("tilemax_layout") == "fullwidth"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("tilemax_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");
	...
	$("ul.layout-picker a").on('click',function(e){
		...
		$.cookie("tilemax_layout", $this.attr("id"), { path: '/' });

		if( $.cookie("tilemax_layout") === "boxed") {
			$("body").removeClass("layout-wide").addClass("layout-boxed");
			$("div#pattern-holder").slideDown();
			
			if( $.cookie("tilemax_pattern") == null ){
				$("ul.pattern-picker a:first").addClass('selected');
				$.cookie("tilemax_pattern","pattern1",{ path: '/' });
			}else{
				$("ul.pattern-picker a[id="+$.cookie("tilemax_pattern")+"]").addClass('selected');
				$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("tilemax_pattern")+".jpg";
	...
	$("ul.pattern-picker a").on('click',function(e){
		if($.cookie("tilemax_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			$.cookie("tilemax_pattern", $this.attr("id"), { path: '/' });
			$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("tilemax_pattern")+".jpg";
			$('body').css('background-image', 'url('+$img+')');
		}
		e.preventDefault();
	...
	$("ul.color-picker a").on('click',function(e){
		...
		$this.addClass("selected");
		$.cookie("tilemax_skin", $this.attr("id"), { path: '/' });
