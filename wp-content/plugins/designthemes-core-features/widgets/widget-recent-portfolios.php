<?php
class TileMax_Portfolio_Widget extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_popular_entries', 'description'=>esc_html__('To list out portfolio items', 'desingthemes-core'));
		parent::__construct(false,TILEMAX_THEME_NAME.esc_html__(' Portfolio','desingthemes-core'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <!-- Form -->
        <p><label for="<?php echo ($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','desingthemes-core');?> 
	    ...
	    <p><label for="<?php echo ($this->get_field_id('_post_categories')); ?>">
			<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','desingthemes-core');?></label>
            <select class="widefat" id="<?php echo ($this->get_field_id('_post_categories').'[]');?>" 
                ...
                <option value=""><?php esc_html_e("Select",'desingthemes-core');?></option>
	    ...
	    <p><label for="<?php echo ($this->get_field_id('_post_count')); ?>"><?php esc_html_e('No.of posts to show:','desingthemes-core');?></label>
			 ...
			 if($the_query->have_posts()) :
				 while($the_query->have_posts()):
					...
					echo "<li>";
						...
						$image = ( $image != false)? $image[0] : TILEMAX_THEME_URI . '/images/dummy-images/post-thumb.jpg';
			 	...
			 	echo "<li>".esc_html__('No Portfolio Entries found','desingthemes-core')."</li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";			 
		echo ($after_widget);
